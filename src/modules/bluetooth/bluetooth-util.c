...
void pa_bluetooth_transport_set_speaker_gain(pa_bluetooth_transport *t, uint16_t value) {
    ...
    backend->set_speaker_gain(y->profiles[t->profile].backend_private, t, PA_MIN(value, HSP_MAX_GAIN));
}
...
static void bluez_backend_set_speaker_gain(void *bp, pa_bluetooth_transport *t, uint16_t value)
{
    dbus_uint16_t gain = value;
...
pa_bluetooth_backend bluez_backend = {
    ...
    .set_speaker_gain = bluez_backend_set_speaker_gain,
};
